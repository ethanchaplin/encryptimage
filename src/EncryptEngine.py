...
class EncryptEngine:
    ...
    def encrypt(
        ...
        prevLocation = 0
        s = data.encode('utf-8')
        c = self.colorify(s.hex())  # takes the encoded string and converts it to hex,

        # and then splices it into sections of six
            ...
            pixels[self.locate(img.size[1], img.size[0], location)[0],
                   self.locate(img.size[1], img.size[0],
                               location)[1]] = (self.hexToRGB(c[dataPoint])[0],
                                                ...
                                                self.hexToRGB(c[dataPoint])[2])  # changes color of selected pixel to

            # corresponding colorified hex value
    ...
    def decode(self, image):
        ...
        for x in range(numIterations):
            ...
            final.append(self.rgbToHex(pixels[self.locate(img.size[1],
                                              ...
                                              self.locate(img.size[1], img.size[0],
                                                          location)[
                                                  1]]))  # appends all data points gathered into one hexadecimal number
