...
def encrypt(data, image, output):
    ...
    for x in c:
        ...
        pixels[locate(img.size[1], img.size[0], location)[0],
               locate(img.size[1], img.size[0], location)[1]] = \
            (hexToRGB(c[dataPoint])[0], hexToRGB(c[dataPoint])[1],
             hexToRGB(c[dataPoint])[2])
...
def decode(image):
    ...
    for x in range(numIterations):
        location = primes[dataPoint] + prevLocation
        final.append(rgbToHex(pixels[locate(img.size[1], img.size[0],
                     location)[0], locate(img.size[1], img.size[0],
                     location)[1]]))
        prevLocation = location
        dataPoint = 1 + dataPoint
    for y in final:
